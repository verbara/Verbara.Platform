...
/// <see cref="IPushEventBus"/> (when present) so new Verbara.Sdk.Push-native consumers
...
public sealed class PlatformEventBus : IDisposable
{
    ...
    /// <see cref="IPushEventBus"/> so Verbara.Sdk.Push subscribers receive the same
...
/// so the Sdk.Push <see cref="Verbara.Sdk.Push.Delivery.DefaultDeliveryFilter"/>
