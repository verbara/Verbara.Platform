...
**Última actualización:** 2026-05-17 · **Baselines actuales:** Platform **`2.1.0`** · Platform.Web `3.0.1` · SDK pin `2.1.2` · Pro pin **`2.3.0-pro`** · **🎉 visibility flip EXECUTED 2026-05-10 19:04 UTC — all 7 ADR-0018 triggers GREEN; Platform + Web repos PUBLIC; first cosign-signed image live at `ghcr.io/verbara/platform/api`** · **✅ 2026-05-16/17: R5.5 Phase 0LK gap-fix rounds 1-3 + B-LK.1/2/3/4/5 all SHIPPED (5 commits in `ce17edc0`..`876e2865`). NBomber envelope mapped (1 075 RPS reads @ p99 97 ms, 3 RPS Argon2id logins). SIP SBC chain validated end-to-end (50/50 OK via Kamailio→Cilium-LB→Asterisk). 3 round-4 gaps documented (Cilium L2 hairpin, postStart timing, SIP retransmit tuning). Comparison-vs-Docker deferred to Phase B-C (cloud, host-equivalent hw).**
