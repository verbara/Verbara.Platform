...
## Addendum — P2 re-decomposition into P2b / P2c / P2d (2026-06-14, append-only)

A deep, code-grounded audit of the shipped P2a baseline (5-agent parallel sweep) found the inherited "P2b" scope (AutoApply + entity prefill + voice enrichment + per-tenant LLM config) was *forward-path only* and could not ship as a single phase "for the final product, no shortcuts." Two findings forced a re-decomposition (no change to the P0–P2a Accepted decisions). Full design in [`docs/specs/2026-06-14-typification-p2b-autoapply-safe.md`](../specs/2026-06-14-typification-p2b-autoapply-safe.md).

- **Voice enrichment is architecturally infeasible at wrap-up time → its own phase P2d.** The wrap-up form opens on `CallEndedEvent`; Pro `CallAnalyticsEngine` *starts* (fire-and-forget) on the same event and finishes seconds later or is dropped under load, so a synchronous fetch sees nothing ~100% of the time. Compounding hard blockers: no stored `Conversation→SessionId` linkage (`Conversation.VoiceLinkedId` = Asterisk LinkedId ≠ CallAnalytics key `SessionId`); `CallSummary.DispositionCode` is declared-but-never-assigned (dead) and no real `CallSummarizer` is wired; the transcript is an in-memory accumulator disposed after analysis. P2d requires Pro prerequisites (persist the id link, wire a real summarizer + durable transcript) **and** a post-call async trigger (subscribe to `CallAnalysisCompletedEvent` → late suggestion via SSE / AutoApply-after-analysis). Typification must not take a Pro dependency (open-core layering); the Api is the bridge.

- **AutoApply needs a trust/safety/cost/observability substrate that is the bulk of the work → stays in P2b, treated as load-bearing.** Key gaps found in P2a: AutoApply `Mode` is persisted but never enforced server-side (client-trust); no RBAC permission to authorize autonomous classification; customer transcript is raw, undelimited LLM input (prompt-injection steering to a valid-but-wrong leaf, dangerous once no human confirms); `/typify` is entirely unaudited and there is no non-human actor concept (GDPR Art. 22); token `usage` is discarded so cost is unmeasurable and there is no budget/LLM-rate-limit (AutoApply = an automatic paid call per conversation, no ceiling); the `llm.completions` resilience policy is never registered so the provider silently runs on `NoOp` (no circuit-breaker/retry, contradicting P2a §3.1); zero metrics/logs on the whole LLM path; the AI's suggested leaf is never stored so per-leaf accuracy is uncomputable and provenance is forgeable. **The load-bearing conclusion: AutoApply cannot be enabled safely without a calibration mechanism (shadow mode + server-side reconciliation proving accuracy first).**

**Resulting phase split (supersedes the single "P2" row of the Phasing table for the AI track; P3/P4 unchanged):**

| Phase | Title | Repos | Delivers |
|---|---|---|---|
| **P2a** ✅ | AI suggest (provider + SuggestOnly) | Platform + Web + Pro | Shipped 2026-06-10 (v2.12.0 / v3.8.0-web / Pro v2.8.0-pro). |
| **P2b** | AutoApply (safe) + entity prefill — **digital/text** | Platform + Web | Graduated bands (D-A) + shadow/calibration gate (D-B) + injection hardening (D-C) + automated-decision audit (D-D) + cost/observability/resilience substrate (D-E) + entity prefill with PII policy (D-F) + per-binding AI override (D-G) + multilingual hardening (D-H) + anti-clobber UX (D-I) + `typification:ai:*` RBAC (D-J). Autonomous commit for abandoned wrap-ups is built but ships disabled (permission + calibration + verification gated). |
| **P2c** | Per-tenant LLM config (production) | Platform + Web | Encrypted per-tenant credentials, provider/model selection, BYO-vs-platform-key, fallback-to-default, health-check, per-tenant cost attribution (Billing), data-residency / no-train flags, per-tenant resilience isolation. |
| **P2d** | Voice post-call enrichment | Platform + Web + **Pro** | Pro: persist `Conversation→SessionId`, real `CallSummarizer` emitting `DispositionCode` + durable/queryable transcript. Platform: post-call async trigger (`CallAnalysisCompletedEvent`) + late suggestion via SSE / AutoApply-after-analysis. |

The accuracy/quality **dashboard** (visualizing the calibration/feedback signal P2b *captures*) lands in **P4**. Streaming suggestions are **cut** (YAGNI — single fast round-trip).
