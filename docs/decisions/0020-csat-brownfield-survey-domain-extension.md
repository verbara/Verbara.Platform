...
- [`src/Verbara.Platform.Storage.Postgres/Migrations/001_InitialSchema.sql:467-488`](../../src/Verbara.Platform.Storage.Postgres/Migrations/001_InitialSchema.sql) — `surveys` + `survey_responses` tables exist (PK `(tenant_id, response_id)`, indexed on `conversation_id` + `survey_id`; answers stored as JSONB).
