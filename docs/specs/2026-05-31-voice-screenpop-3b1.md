...
## 9. Phase C outcome (agent-assist per-conversation, D3 = INCLUDE — 2026-05-31)

Web-only (no Platform/Pro change — the A+B backend already enriches every `AgentAssist*Event` with `ConversationId`). Delivered exactly as §4.4:
- **`agent-ai-store` refactored global → per-conversation keyed** (`sessions: Record<conversationId, AgentAiSession>`). Every action takes a leading `conversationId`; a stable `EMPTY_SESSION` fallback keeps zustand selectors reference-stable (no re-render loop). Dropped the global `isActive` flag in favor of the derived `sessionHasData(session)`.
- **`use-sse` routes `agentassist.suggestion|sentiment|compliance_alert|transcript` by `data.conversationId`** into the right slice (still gated by `isForCurrentAgent`). A missing `conversationId` skips the store write (no bleed into an arbitrary conversation) but still fans out to manual handlers — fail-safe for legacy/malformed events.
- **`SuggestionBanner` / `SentimentGauge` / `ComplianceAlert` / `TranscriptTab` take a `conversationId` prop** and read their own slice; `conversation-panel` passes the conversation's id; `context-panel` reads `selectedId` and gates the transcript tab on `sessionHasData` for that conversation. A voice call and a concurrent digital conversation no longer bleed agent-assist.
- **Lifecycle cleanup (self-review finding):** the keyed store would otherwise accumulate the AI session of every closed conversation for the tab's lifetime (the old global store was size-bounded). `conversation-store` now calls `clearSession(conversationId)` on the **terminal** `state_changed` transition (alongside the existing `removeConversation`). WrapUp is NOT terminal, so the transcript/sentiment stay visible while the agent dispositions the call.
- **Supervisor monitor (`SessionDetail`):** has no per-conversation scoping, so it flattens across whatever sessions exist in its browser via `flattenAiSessions` (single-session → verbatim; multi → concat lists + latest sentiment by timestamp). Documented caveat: the supervisor's browser does not receive the *monitored* agent's `agentassist.*` (filtered to its own agentId) — server-side fan-out of the monitored session is future work; flatten preserves prior behavior without regressing.
- **Tests:** `agent-ai-store.test.ts` (16: per-conversation isolation of all 4 actions + caps + scoped dismiss/ack + `clearSession` scoping/no-op + `sessionHasData` + `flattenAiSessions` single/merge/latest-sentiment) + `suggestion-banner.test.tsx` (2: component shows only its conversation's suggestions). Suite **1062 → 1080** green; eslint 0 errors (also fixed the latent A+B `react-hooks/set-state-in-effect` error on the wrap-up effect via the repo's idiomatic suppression + justification); tsc + production build clean; i18n parity OK (no new keys — reuses `ai.*`).
