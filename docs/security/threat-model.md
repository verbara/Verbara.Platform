...
- **2026-05-23 (image-public is orthogonal to repo-public — ADR-0023 Accepted)**: this threat model's §3 "What going public exposes" is **repository-flip** focused (ADR-0018 Trigger 4 framing). It does NOT cover the *image-visibility* question, which has its own decision train [ADR-0023](../decisions/0023-publishing-non-aot-microservices.md) (Accepted 2026-05-23). Status update addendum:
  ...
  - **§8 Open risks** — the "LicenseGateMiddleware bypassable by source patch" row predates Pro/ADR-0011 Layer C. Effective new state: a source patch + rebuild produces a new image digest that won't match the license-claim authorized list → bypass works only for OSS-mode features (which are free anyway). The economic incentive for bypass collapses; technical detection via Layer C makes "I ran the genuine image" verifiable by digest. Re-evaluation trigger row updates to "first incident of detected commercial bypass" (which now requires forging Layer-A native-RE + Layer-B ECDSA + Layer-C digest binding simultaneously).
...
- **2026-07-29 (A7 — MFA enrollment material is now DataProtection-wrapped at rest; boundary stated)**: change `encrypt-mfa-secrets-at-rest` (`decision_ref` [ADR-0003](../decisions/0003-dataprotection-key-persistence-strategy.md)) closes the last unwrapped column-level secret Platform persisted. Updates by section:
  - **§2 Assets, row A7 (MFA enrollment material — TOTP shared secrets, recovery codes; High)** — the row's "why it matters" was written around *recovery without audit*; it did not record that the material was also persisted **unwrapped**. `users.mfa_secret` held the raw Base32 TOTP shared secret verbatim (anyone reading the column could mint valid codes for that user indefinitely, silently, with no audit event), and `users.mfa_recovery_codes` held digests in two coexisting formats, one of them a single-round salted SHA-256 hex over a ~40-bit keyspace whose salt (`users.user_id`) lives in the same row. NOW BOTH ARE ✅ **DataProtection-wrapped at rest** by protect-on-write / unprotect-on-read in `PostgresUserStore` under two concern-specific purposes — `Verbara.UserMfaSecret` and `Verbara.UserMfaRecoveryCodes`, the array wrapped element-wise so the column stays `TEXT[]` — plus a one-shot idempotent `UserMfaEncryptionMigrator` that converts pre-existing rows on boot. Same protect/unprotect/trial-unwrap shape as the ADMIN-001 OIDC fix recorded in the 2026-05-09 (later) entry above; no schema change, no API change, no hash-format change. The canonical column → purpose list is the **protected-column register** in ADR-0003 (addendum of even date).
  - **§6.4 Information disclosure** — the row *"TA1 reads plaintext secrets from DB after stealing a backup — A4 — Every secret column wrapped via `IDataProtectionProvider` … ✅ Verified"* was accurate for A4 but **overclaimed twice**, and both corrections belong on the record:
    - **Coverage.** "Every secret column" did not include A7's two columns when that row was written. It does now.
    - **Strength — the residual-risk boundary, stated rather than implied.** That row's own scenario is *"after stealing a backup"*, and the wrap does **NOT** defend against the full-dump case. `AddPlatformDataProtection` exposes no `ProtectKeysWith*` option today, so with the default `UsePostgres` keyring `data_protection_keys` holds key XML **unencrypted in the same database** as every wrapped column. **A complete database compromise that includes `data_protection_keys` is explicitly outside what this wrap mitigates** — such an attacker holds keyring and ciphertext together and the wrap buys nothing. What it does mitigate is **partial** exposure, which is the realistic self-host failure mode: a table-scoped dump (`pg_dump -t users`), a CSV/report extract, a read-replica scoped to application tables, or a SQL-injection read that reaches `users` but not `data_protection_keys`. Status for A7 is therefore ✅ **Verified against partial exposure**, ⚠️ **not mitigated against full-database compromise**. The follow-up that would close it — wrapping the keyring itself with a certificate or KMS — is named in the ADR-0003 addendum and is deliberately not built by this change.
  - **§6.6 Elevation of privilege, row "TA2 bypasses MFA via lost-device recovery" (A7)** — unchanged and still ✅ Verified: recovery remains a per-user admin reset through `MfaAdminService.ResetMfaAsync` (`/management/mfa`), still audit-emitting, still tenant-scoped per the 2026-05-09 MFA-admin fix above. The wrap adds no bypass path: it is byte-for-byte transparent to every caller (`MfaService.VerifyCode`, `MfaService.ValidateRecoveryCode`, `MfaEnrollEndpoints`, `ProfileRecoveryCodesEndpoints`, `MfaAdminService`), none of which changed.
  - **§7 Cross-cutting controls** — the row *"`IDataProtectionProvider` keyring backed by DB — A2, A4, A9"* extends to **A7**. Its coverage cell should be read as A2, A4, A7, A9.
  - **§8 Open risks and residual exposure** — one risk is now **added by** this change and accepted: *"DataProtection keyring durability is load-bearing for MFA, not only for OIDC client secrets and AgentAssist credentials."* Severity **Medium**. Losing the keyring is **silent by design** — unwrappable values fall through verbatim, so `MfaService.VerifyCode` simply returns false rather than raising, and enrolled users can no longer complete TOTP verification. Acceptance rationale: with the default `UsePostgres` keyring, `data_protection_keys` and `users` live in the same database, so any whole-database backup captures both — but a **table-scoped** backup policy may not, which is the trap operators must check for. Recovery is a per-user admin MFA reset via `MfaAdminService` (already audit-emitting). Documented in `docs/operations/backup-disaster-recovery.md` §1.6 and §3.6. Re-evaluation trigger: shipping keyring-at-rest wrapping (certificate or KMS), or the first reported keyring-loss incident.
  - This Status update **does not edit** the §2 A7 row, the §6.4 row, or the §7 row above; per this document's append-only rule readers should treat those as accurate-at-publication-time and consult this entry for current state. A future top-level revision will fold them in line.
- **2026-07-29 (A7 — recovery codes minted by the enrollment wizard were unredeemable; MFA verification failures are now audited and throttled)**: change `fix-recovery-code-redemption` (`decision_ref` [ADR-0013](../decisions/0013-password-hash-algorithm-migration.md)) fixes a defect in the *only* redemption path for MFA recovery codes, and closes the verification half of the Audit Scope 3.4 gap it surfaced. Updates by section:
  - **§2 Assets, row A7 (MFA enrollment material — TOTP shared secrets, recovery codes; High)** — the row's "why it matters" reads *"recovery without audit collapses second-factor protection"*. Two things were true of that material that the row did not record. **First, for one of the two mint families the recovery path did not work at all.** `users.mfa_recovery_codes` carries two digest families — BCrypt cost-10 from the legacy endpoints (`POST /auth/mfa/setup`, `POST /auth/mfa/recovery-codes/regenerate`) and 64-character salted SHA-256 hex from the profile enrollment wizard (`POST /profile/security/mfa/enroll/verify`, `POST /profile/security/recovery-codes/regenerate`) — while the sole redemption surface, `AuthEndpoints.MfaVerify` → `MfaService.ValidateRecoveryCode`, called `BCrypt.Verify` unconditionally in an unguarded loop. A SHA-256 element fails BCrypt's `$2` precondition and raises `SaltParseException`, which derives directly from `Exception` and so fell through `ErrorHandlingMiddleware` to **HTTP 500 with the crypto library's raw `"Invalid salt version"` message in `ProblemDetails.Detail`**. A wizard-minted recovery code was therefore not weak — it was **unredeemable**, which for a credential that exists solely for "I lost my authenticator" means a permanent lockout with no self-service path and an admin MFA reset as the only remedy. **Second, the failure was expensive and misattributed.** `MfaVerify` consumes the one-shot `mfaToken` challenge via `IMfaPendingCache.TakeAsync` — an atomic destructive read — *before* reaching the verify, so the token was burned on the way to the throw and each attempt cost the user a full re-login; and the operator UI special-cases only 429 and 400-with-`expired`, so a 500 rendered as the generic *"invalid code"* and the user was told they had mistyped. NOW ✅ **both families verify.** `ValidateRecoveryCode` classifies each stored element on its own shape (`$2` → BCrypt, anything else → `IRecoveryCodeService.Verify` with salt `user.UserId.Value`), and the BCrypt branch carries the `SaltParseException` → no-match guard that `PasswordService.VerifyPassword` has had since ADR-0013. **No stored value was rewritten, re-hashed, or invalidated** — the fix is entirely in the reader, so codes already in users' password managers keep working, in both families. This change does **not** unify the two families and does **not** strengthen the SHA-256 digest (a single round over a small keyspace); its at-rest exposure is handled separately by `encrypt-mfa-secrets-at-rest`, whose column wrap is byte-for-byte transparent to this dispatch.
  - **§6.6 Elevation of privilege, row "TA2 bypasses MFA via lost-device recovery — A7 — ✅ Verified — Audit Scope 3.4"** — accurate for the half it measured (admin-elevated recovery via `MfaAdminService.ResetMfaAsync`, still audit-emitting and still tenant-scoped), but Scope 3.4 requires *"MFA enrollment **+ verification** audited"* and the verification half was not implemented: `POST /auth/mfa/verify` performed **no lockout bookkeeping and emitted no auth event on any failure path** — not for the 500 above, not for a plainly wrong TOTP code, not for a wrong recovery code — while `Login` had recorded both since it shipped. Second-factor guessing against an `AllowAnonymous` endpoint was therefore **unaudited and unthrottled beyond the global rate-limit bucket**. NOW ✅ **audited and counted**: a failed attempt calls `AccountLockoutService.RecordFailedAttemptAsync` and emits `AuthEventTypes.MfaVerificationFailure` (`mfa_verification_failure`) with IP, user-agent and a factor-naming reason only — the submitted code, the stored digest and the TOTP secret never reach the audit log. Scope 3.4's verification half is closed for this endpoint; the enrollment and admin-recovery halves are unchanged.
  - **§6.5 Denial of service, row "TA1 brute-forces login endpoint — A2 — rate-limited … lockout after N failures"** — that row is about `/api/auth/login` and was never a claim about `/auth/mfa/verify`, which is a *separate* anonymous credential-checking endpoint reached with a one-shot challenge token. It now shares login's lockout accounting. The open question of whether it also warrants its own rate-limit policy (it has none today beyond the global bucket) is recorded in the change's design and is **not** decided here.
  - **Blast radius — stated precisely, because it was latent rather than universal.** The shipped operator UI wires the **legacy** MFA endpoints, so every user who enrolled through it holds BCrypt codes that redeemed correctly; this was not a live incident for the shipped frontend path. The wizard routes were nonetheless **live, authorization-guarded, documented, and present in the generated OpenAPI surface** — reachable by direct URL and by any non-web API client, so any such client's enrollees were affected. The exposure was therefore latent and self-growing: **the moment the enrollment wizard is linked from frontend navigation, every new enrollee's recovery codes become unredeemable.** Wiring that navigation before this fix ships is explicitly gated against in the change's migration plan.
  - This Status update **does not edit** the §2 A7 row, the §6.5 row, or the §6.6 row above; per this document's append-only rule readers should treat those as accurate-at-publication-time and consult this entry for current state. A future top-level revision will fold them in line.
