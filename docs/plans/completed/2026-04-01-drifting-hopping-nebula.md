...
El Docker build de Platform API falla con `CS0246: ILicenseStatus could not be found`. La causa raíz es que `Asterisk.Platform/local-nuget-feed/` (la copia dentro del repo, usada por Docker) tiene paquetes Pro del **30 de marzo**, pero `ILicenseStatus` se agregó al paquete Pro.Licensing en el **1 de abril** (Plan 30A). El feed compartido `/media/Data/Source/Verbara/local-nuget-feed/` sí tiene los paquetes actualizados.
...
|-----------|-------|---------------------|
| `/media/Data/Source/Verbara/local-nuget-feed/` (shared) | 2026-04-01 | Si |
| `/media/Data/Source/Verbara/Asterisk.Platform/local-nuget-feed/` (repo) | 2026-03-30 | No |
...
```bash
cp /media/Data/Source/Verbara/local-nuget-feed/Asterisk.Sdk.Pro.*.nupkg \
   /media/Data/Source/Verbara/Asterisk.Platform/local-nuget-feed/
```

### Paso 2: También sincronizar paquetes Sdk (por si hay desync)

```bash
cp /media/Data/Source/Verbara/local-nuget-feed/Asterisk.Sdk.*.nupkg \
   /media/Data/Source/Verbara/Asterisk.Platform/local-nuget-feed/
```
...
- `/media/Data/Source/Verbara/Asterisk.Platform/local-nuget-feed/` — paquetes desactualizados
- `/media/Data/Source/Verbara/local-nuget-feed/` — paquetes actualizados (fuente de verdad)
- `/media/Data/Source/Verbara/Asterisk.Platform/Dockerfile` — Docker build
- `/media/Data/Source/Verbara/Asterisk.Platform/NuGet.Config` — referencia al feed local
