# audit-trail-integrity Specification

## Purpose
Guarantees the platform's audit trail is trustworthy end-to-end: corrections and their audit
records land atomically (no orphan correction survives a crash), GDPR purge previews report a real,
queryable audit-row count rather than a placeholder, every `RecordAudit` call-site attributes the
actor through the one canonical resolution (impersonating operator, not the impersonated subject;
API-key identity, never a `"system"` fallback), and the integrity hash covers `RetainUntil` under a
versioned scheme so retention-date tampering is detectable without invalidating entries written
before the scheme changed. These four defects were identified and deferred at the
typification-autonomous-disposition ship and closed here (Platform/ADR-0034).
## Requirements
