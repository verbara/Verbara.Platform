# released-image-smoke Specification

## Purpose
Verifies that a just-released, cosign-signed set of Platform images actually boots and functions
together, not merely that their signatures are valid — the existing
`docker/verbara-verify-image.sh` proves provenance only. Brings up the demo compose stack pinned to
the release's real digests, waits on binary health signals (never a wall-clock sleep), and runs one
walking-skeleton end-to-end journey; a released image that boots but cannot complete that journey
fails the check and flags the release, distinct from a passing signature check. Scope stays inside
the Platform repo — a shared cross-repo E2E harness is an explicit, separately-decided future step,
not scaffolded here (verbara-meta ADR-0006).
## Requirements
