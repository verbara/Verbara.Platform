...
- [ ] 1.1 Add `MfaVerificationFailure = "mfa_verification_failure"` to `AuthEventTypes`
...
- [ ] 1.2 Elevate `AuthEndpoints.MfaVerify` from `private static` to `internal static`, carrying a
...
- [ ] 2.1 Change the signature to
  ...
  is static (design D4).
- [ ] 2.2 Dispatch **per element** (design D1), mirroring the shape of
...
- [ ] 2.3 Wrap the BCrypt branch in `try { … } catch (BCrypt.Net.SaltParseException) { /* no match */ }`
...
- [ ] 2.4 Add a `private const string BcryptPrefix = "$2";` and an XML doc on the method spelling out
...
- [ ] 2.5 In `AuthEndpoints.MfaVerify`, add `[FromServices] IRecoveryCodeService recoveryCodes` to the
...
- [ ] 2.6 Add the failure bookkeeping the handler has never had (design D6, spec: "Every failed
  ...
  needed for either.
- [ ] 2.7 Distinguish the failure reason in the event payload (e.g. `invalid_totp`,
...
- [ ] 2.8 Confirm the success path still resets the attempt counter and emits its success event via
  `IssueTokensAsync` — do not duplicate that bookkeeping on the success branch.

## 3. Phase C — Integration (batch)

- [ ] 3.1 Update the only other caller of `ValidateRecoveryCode` —
...
- [ ] 3.2 Verify `IRecoveryCodeService` is resolvable wherever `MfaVerify` runs. It is registered
...
- [ ] 4.1 `MfaService`-level tests in `tests/Verbara.Platform.Api.Tests/Services/MfaServiceTests.cs`,
...
- [ ] 4.2 End-to-end mint→redeem tests, one per mint path (spec: "Each mint path has an end-to-end
...
- [ ] 4.3 `MfaVerify_ShouldRejectReplay_WhenSameRecoveryCodeUsedTwice` — the second redemption of an
  already-used code fails, pinning one-time use across both families.
- [ ] 4.4 `MfaVerify_ShouldReturn401NotServerError_WhenStoredDigestIsMalformed` — assert the status is
...
- [ ] 4.5 Bookkeeping tests: a failed redemption writes an auth event of type
...
- [ ] 4.6 Correct the misleading comment in
...
- [ ] 4.7 Replace or annotate the `MfaRecoveryCodes = new[] { "code1", "code2" }` placeholder fixtures
...
- [ ] 5.1 Append an addendum to `docs/decisions/0013-password-hash-algorithm-migration.md` recording
...
- [ ] 5.2 Add a status update to `docs/security/threat-model.md` under asset **A7**: recovery codes
...
- [ ] 5.3 `CHANGELOG.md` `[Unreleased]` — a `### Fixed` entry for the redemption defect (naming the
