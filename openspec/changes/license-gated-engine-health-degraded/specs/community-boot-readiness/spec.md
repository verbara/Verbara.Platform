...
- **Level:** LOW
- **Affected:** Platform's `/health/ready` behavior on unlicensed community / self-host boots
  (readiness/load-balancer join); the released-image smoke leg in `.github/workflows/release.yml`.
  No Platform production source (`src/Verbara.Platform.Api/Health/*`, `Program.cs`) changes — the
  aggregate flip is entirely driven by the Verbara.Sdk.Pro pin bump (Verbara.Sdk.Pro/ADR-0017).
  The only cross-repo coupling is the `Verbara.Sdk.Pro.*` `2.12.0-pro` → `2.13.0-pro` pin.
- **Mitigation:** the readiness change is a strict severity de-escalation (Unhealthy → Degraded) for
  the license-blocked branch only — a stalled LICENSED engine stays `Unhealthy`, so a real
  operational fault still fails readiness. The integration test and the sharpened, gating-after-two
  smoke leg both pin the exact consumed contract (`dialer-engine` `Degraded` + `dialer license
  blocked:` prefix), so a producer or middleware regression is caught in Platform CI. Graduating
  the smoke leg to gating only after two consecutive green runs bounds the blast radius of a flaky
  new stage per the released-image-smoke walking-skeleton posture.
