...
- [ ] 1.1 Bump the `Verbara.Sdk.Pro.*` package pins from `2.12.0-pro` to `2.13.0-pro` (the version
...
- [ ] 1.2 (apply-stage, NOT this propose-only child) Run the pin cascade at `/xr:apply` — producer
...
- [ ] 2.1 Add an integration test that boots Platform in an unlicensed / community configuration
  (Postgres configured, no valid Pro license) and issues `GET /health/ready`, asserting the HTTP
  status is **200** (NOT 503).
- [ ] 2.2 In the same test, parse the JSON body emitted by
  `src/Verbara.Platform.Api/Health/HealthReportJsonWriter.cs` and assert the top-level `status` is
  `Degraded`.
- [ ] 2.3 Assert the `checks` object's `dialer-engine` entry has `status` == `Degraded`.
- [ ] 2.4 Assert that entry's `description` STARTS WITH the prefix `dialer license blocked:`.
...
- [ ] 2.5 Name the test per convention `Method_ShouldExpected_WhenCondition`
...
- [ ] 3.1 Sharpen `docker/verbara-smoke-released.sh` so the community (unlicensed) boot leg asserts
...
- [ ] 3.2 After the sharpened community smoke leg has run green **twice consecutively** while
  report-only, graduate it in `.github/workflows/release.yml` from report-only
  (`continue-on-error: true`) to **gating** — drop `continue-on-error` from that leg and/or make a
  later step `needs:` it. (Follow-up after the first two green runs; not merged with 3.1.)

## 4. CHANGELOG

- [ ] 4.1 Add a `[Unreleased]` entry to `CHANGELOG.md` recording the consumed contract flip
  ...
  the sharpened/gating smoke leg. Cite `decision_ref: Verbara.Sdk.Pro/ADR-0017`.

## 5. Out of scope (record, do not implement)

- [ ] 5.1 Confirm NO change is made to any Platform health-check source
...
- [ ] 5.2 Confirm the licensed-profile smoke leg is NOT built here (separate follow-up change).

## 6. Verification

- [ ] 6.1 `dotnet test` green with zero warnings (TreatWarningsAsErrors=true, WarningLevel=9999),
  including the new community-boot readiness integration test.
- [ ] 6.2 `openspec validate --all --strict` green.
- [ ] 6.3 CI green on the PR (Platform gate set: build, tests, OpenSpec validation, and any
  invariant scripts).
- [ ] 6.4 The released-image smoke community leg is green twice consecutively before the gating
  promotion in 3.2 lands.
