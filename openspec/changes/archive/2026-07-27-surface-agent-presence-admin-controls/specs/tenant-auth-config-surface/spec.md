...
**Affected:** `Verbara.Platform` (the two tenant auth-config DTOs on
`AuthAdminEndpoints.cs` / `TenantAuthConfigResponse.cs` and the `UpdateConfig` handler) and,
downstream, `Verbara.Platform.Web` (binds a `pendingPauseTimeoutMinutes` admin control to the surfaced
field — the child change). No effect on `Verbara.Sdk` or `Verbara.Sdk.Pro` (neither consumes the
tenant auth-config HTTP surface).

**Mitigation:** the change is a pure surfacing of an already-persisted, already-consumed model field —
one nullable `int?` on the write DTO plus one handler line, and one `int` on the read DTO plus its
`FromConfig` projection line, both mirroring the shipped `sessionIdleTimeoutMinutes` handling verbatim.
No new endpoint, no store change, no migration, no `ApiJsonContext` addition (both DTOs are already
registered). AOT-safe (typed sealed records, source-gen serialization, no reflection). The redacted
OIDC-secret surface is untouched. Web binds its control only after Platform surfaces the field (hard
buildOrder barrier, `impact.yaml`), and every wire field name is pinned verbatim to the golden fixtures
(`fixtures/tenant-auth-config-update-request.json`, `fixtures/tenant-auth-config-response.json`).
