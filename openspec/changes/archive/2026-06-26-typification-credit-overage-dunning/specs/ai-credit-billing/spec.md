...
**Affected:** `Verbara.Platform.Billing` (invoice generation +`ITenantQuotaStore` dep, `DunningConfig`,
new issuance worker); `Verbara.Platform.Storage.Postgres` (invoices schema migration + store round-trip);
`Verbara.Platform.Api` (metering hook, DI + config binding, `DueDate` at issue); `DunningService`
(unchanged logic, but now actually fires under Postgres).

**Mitigation:** The schema migration is additive (two nullable/defaulted columns) and backward-compatible
(legacy rows default to `Current`/`null`, preserving today's skip-if-null dunning behaviour). The overage
line item reuses the existing `OverageQuantity`/`IncludedQuantity` fields; only the basis (credits vs
tokens) changes, and only for `AiAnalysis`. The threshold hook is stateless (straddle detection) so it
adds no table and no distributed lock; double-notification is further guarded by `NotificationService`'s
5-minute dedup window. The issuance worker mirrors the proven `DunningService` hosted-service template and
issues only overage drafts. Tenant-wide automatic invoice generation is explicitly out of scope. No
Sdk/Pro changes; `TenantStatus` is consumed, not extended.
