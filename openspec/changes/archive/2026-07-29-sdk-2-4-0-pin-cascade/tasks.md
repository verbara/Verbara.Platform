...
- [x] 1.1 Confirm `Verbara.Sdk` **`2.4.0`** is published (tag **`v2.4.0`**, 2026-07-26). This leg is
...
- [x] 1.2 Confirm the `Verbara.Sdk.Pro` leg (`buildOrder: 2`) has landed its own cascade — its
...
- [x] 1.3 Run the cross-repo sequence for the Pro hop: **edit Sdk/Pro → `dotnet pack` to the local
...
- [x] 2.1 Bump the **7** direct `Verbara.Sdk.*` pins in `Directory.Packages.props` from **`2.3.2`**
  ...
  both.
- [x] 2.2 Bump the **22** direct `Verbara.Sdk.Pro.*` pins in `Directory.Packages.props` lines
  **50-71** from **`2.13.0-pro`** to **`2.13.1-pro`**.
- [x] 2.3 Bump the **7** direct `Microsoft.Extensions.*` pins in `Directory.Packages.props` lines
...
- [x] 2.4 **Commit 2.1 + 2.2 + 2.3 together, in ONE commit** (`Verbara.Sdk/ADR-0040` D2). Every
...
- [x] 2.5 **Prefix-hazard check before committing:** confirm no `Verbara.Sdk.Pro.*` pin was set to
...
- [x] 2.6 Confirm the edit touches **only** `Directory.Packages.props` — **no** file under `src/` or
...
- [x] 3.1 Confirm `Microsoft.Extensions.TimeProvider.Testing` is **still `10.0.0`** at
...
- [x] 3.2 Confirm **no direct pin was added** for `OpenTelemetry` or `NATS.Client.Core`. Platform
...
- [x] 3.3 Confirm **no version-asserting script, test or CI step** was added. This repo has none by
  design; verification is the existing required gate set (§5).
- [x] 3.4 Confirm this cascade is **not** bundled with a release/version cut
  (`Verbara.Sdk/ADR-0040` D3) — no `CHANGELOG.md` version heading is cut, no tag is pushed.

## 4. CHANGELOG (apply-stage)

- [x] 4.1 Add a `[Unreleased]` entry to `CHANGELOG.md` under this repo's existing `### Dependencies`
...
- [x] 5.1 `dotnet restore` clean — **zero** `NU1605` / `NU1109` package-downgrade errors. This is
  the gate that proves the D2 same-commit rule was honoured.
- [x] 5.2 `dotnet build` green with **zero warnings** (`TreatWarningsAsErrors=true`,
  `WarningLevel=9999`).
- [x] 5.3 `dotnet test` green with **zero warnings**.
- [x] 5.4 The required **`AOT Publish (Api)`** job (`aot-probe`,
