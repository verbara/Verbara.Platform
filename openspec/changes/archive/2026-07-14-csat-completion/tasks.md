...
- [x] 8.3 `src/Verbara.Platform.Api/Services/DeferredPrimaryAmiConnection.cs` (NEW) + `Program.cs` — voice `IAmiConnection` deferred to first use (design D9): the orchestrator BackgroundService constructs the voice adapter during `Host.StartAsync`, so an eager throwing factory crashed every headless / no-telephony boot (CI OpenAPI-export capture). Fail-at-use wrapper over `VerbaraServerPool.GetServer("primary")` (mirrors `AmiDtmfSource`); regression in `DeferredPrimaryAmiConnectionTests` + `CsatRunnerWiringTests` (no-AMI boot resolves orchestrator + voice adapter without throwing)
