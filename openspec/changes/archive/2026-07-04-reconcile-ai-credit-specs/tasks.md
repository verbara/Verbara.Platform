...
- [x] 2.1 Apply the requirement deltas to `openspec/specs/**` (archive flow applies them; verify the
...
- [x] 3.1 `openspec validate --specs --strict` green (all 7 specs)
- [x] 3.2 Spec-vs-code check of every rewritten requirement: `PostMeteredDebitAsync` signature
...
- [x] 3.3 No remaining intra-spec contradiction: grep the living specs for "inert", "coveredSource",
      "lands with the partner-scoped endpoint" — all gone
- [x] 3.4 docs(openspec) PR green (docs-only; CI gates pass) and merged via merge queue

## 4. Closing

- [x] 4.1 Archive the change (`openspec archive reconcile-ai-credit-specs`) — checkboxes first,
