...
- **Level:** MEDIUM
- **Affected:** `Verbara.Platform.Api` endpoint handlers in the four converted groups; the emitted
  `/openapi/v1.json` document consumed by `Verbara.Platform.Web` codegen. No Sdk/Pro impact; no
  storage or middleware impact.
- **Mitigation:** phased, group-by-group conversion keeps blast radius per phase small; wire-body
  parity is an explicit invariant (existing endpoint/integration tests guard against contract
  regressions); the manifest + extended `verify-openapi-fixture.py` gate catches any emitted-schema
  drift in CI; a handler class that the typed-signature route cannot express is recorded as an open
  question (see `design.md`) rather than silently forcing a document transformer.
