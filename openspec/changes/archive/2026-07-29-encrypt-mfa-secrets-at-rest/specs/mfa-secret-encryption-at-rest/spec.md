...
**Affected:**
- `src/Verbara.Platform.Storage.Postgres/Stores/PostgresUserStore.cs` — the hottest store in the
  auth path (`POST /auth/login`, `GET /auth/me`, every RBAC resolution behind `CachedUserStore`).
  A defect in the projection breaks authentication for every user, not only MFA-enrolled ones.
- Every MFA consumer — `AuthEndpoints` (login MFA challenge, legacy setup/confirm/disable),
  `MfaEnrollEndpoints`, `ProfileRecoveryCodesEndpoints`, `MfaAdminService`, `MfaPolicyEnforcement`.
  None of them changes, but all of them break if the projection returns a wrong value.
- `src/Verbara.Platform.Api/Program.cs` composition root — one added registration.
- Operations: DataProtection keyring durability becomes load-bearing for MFA, not only for OIDC
  secrets and AgentAssist credentials.
- **Cross-repo: none.** `Verbara.Sdk` and `Verbara.Sdk.Pro` are untouched, no pin moves, and
  `Verbara.Platform.Web` sees no contract change — the wrapped values never cross the HTTP boundary
  in either direction.

**Mitigation:**
- The pattern is not novel: `PostgresTenantAuthConfigStore` + `OidcClientSecretEncryptionMigrator`
  have carried the identical protect/unprotect/trial-unwrap shape in production since the ADMIN-001
  fix. This change follows it rather than inventing a second mechanism.
- The verbatim fallback on `CryptographicException` makes the rollout order-independent: legacy rows
  authenticate before the migrator reaches them, and a rollback to the previous binary still reads
  wrapped rows as opaque strings without throwing — the only casualty is that MFA verification fails
  for already-migrated users until the binary is rolled forward again, which the deploy runbook must
  call out.
- Idempotence plus per-element trial detection makes a crash mid-migration self-healing: the next
  boot converges the partially wrapped array with no manual intervention.
- The regression suite pins the contract at the store boundary — column-never-equals-plaintext,
  round-trip equality, legacy fallback, mixed-array convergence, migrator idempotence — against a
  real Postgres via Testcontainers, matching the existing ADMIN-001 suite.
- Scope discipline caps the blast radius: no schema change, no API change, no hash-format change, no
  caller edits. The adjacent recovery-code redemption-format defect is documented and deliberately
  left untouched so this change cannot be blamed for, or entangled with, it.
