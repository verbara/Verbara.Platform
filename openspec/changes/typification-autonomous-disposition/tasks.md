...
- [x] 6.1 Add activation-gate endpoints `POST /admin/typification/autonomous-disposition` and `DELETE /admin/typification/autonomous-disposition` (authorize `AdminOnly`); delegate to `ITenantAutonomousDispositionStore`; return 201 / 204. POST builds a FRESH non-revoked record so re-activation clears a prior revocation (B1 review)
- [x] 6.1a Migration `015_typification_submission_corrections.sql`: create `typification_submission_corrections` (`tenant_id TEXT NOT NULL`, `conversation_id TEXT NOT NULL`, `corrected_leaf_node_id TEXT NOT NULL`, `corrected_node_path JSONB NOT NULL`, `corrected_by_user_id TEXT NOT NULL`, `corrected_at TIMESTAMPTZ NOT NULL`, `PRIMARY KEY (tenant_id, conversation_id)`); idempotent. Add `TypificationSubmissionCorrection` row type + `ITypificationSubmissionCorrectionStore` (Get/Insert) + Postgres (`::jsonb` cast, explicit `NpgsqlDbType`) + InMemory impls + DI
- [x] 6.2 Add supervisor correction endpoint `POST /conversations/{id}/typification-correction` (authorize a new `typification:correct-autonomous` permission via the `Permission:` policy prefix). Guards: the conversation's submission is `Source=AutoAi` (else 409 `NotAutonomous`); `CompletedAt` within `AutonomousCorrectionWindowDays` (else 409 `CorrectionWindowExpired`); not already corrected (else 409 `AlreadyCorrected`) — all BEFORE any write. Effect: INSERT the separate correction record; flip the submission's `CorrectionState=Corrected` + `CorrectedAt` (status pointer — the AI leaf/path/confidence/Source stay UNCHANGED); conversation stays `Closed` (no reopen); emit `typification.autonomous.corrected` audit (actor = supervisor user, original AI path + corrected path, `Confirmed = original==corrected`) with `RetainUntil`; increment `autonomous_corrections_total`
- [x] 6.3 Define DTOs as typed sealed records — `AutonomousDispositionGateRequest`, `AutonomousDispositionGateResponse`, `TypificationCorrectionRequest`, `TypificationCorrectionResponse` (+ `TypificationCorrectionErrorResponse` for the machine-readable 409 codes) — and register them in `ApiJsonContext`
- [x] 6.4 Add `typification:correct-autonomous` permission to `PermissionSeeder.cs`; assign to Supervisor (explicit) + Admin/System Admin (via `AllPermissions()`) role templates in `RoleTemplateSeeder.cs`
- [x] 6.5 Api-layer tests: gate set/revoke (happy + unauthorized non-admin + re-activation clears revocation); correction (happy + `CorrectionWindowExpired` + `NotAutonomous` + `AlreadyCorrected` + unauthorized) — assert the original submission's AI leaf/path/Source stay UNCHANGED and the separate correction record holds the human path
- [x] 6.6 Run `dotnet test` — Api tests green; zero warnings
