...
> - **Release process hardening (2026-05-23 PR #5):** After the v2.4.2 anomaly (4 ghcr.io images pushed via maintainer-local `docker buildx --push`, bypassing `release.yml` — visibility-monitor stayed green for 13h because it only checked anonymous pull, not cosign), `visibility-monitor.yml` now also `cosign verify`s the 5 tagged images + checks `https://verbara.io/keys/cosign.pub` PEM-block parity vs in-repo `.github/cosign.pub`; new `digest-reconciliation.yml` reconciles `verbara-website/data/authorized-digests.json` against ghcr.io daily 07:00 UTC; `release.yml` skips cleanly when the annotated tag message carries `RETROACTIVE-TAG`. **ADR-0024 still pending file.** Implication for v2.4.3 ship (lab migration): **MUST go through `release.yml`**, not buildx-direct, or daily reconciliation fires drift.
