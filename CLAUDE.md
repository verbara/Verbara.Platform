...
> **🔒 AOT shipping ([ADR-0022](docs/decisions/0022-platform-api-aot-shipping-path.md)):** `Verbara.Platform.Api` publishes **Native AOT** (`<IsAotCompatible>true>`; 0 `IL2026`/`IL3050`/`IL207x` diagnostics; native ELF, 0 managed Verbara DLLs). The SDK + Pro libraries AND the Api host are all AOT-compatible. The root `Dockerfile` is the AOT pathway (`runtime-deps` final stage); a Docker AOT build restores Pro from `github`.
...
- **AI / Workflow:** `Flows` (DAG, 11 node types, LLM abstraction), `Llm` (OpenAI-compatible `ILlmProvider`; powers `ai_classify`/`ai_generate` flow nodes + typification AI), `Bot` (virtual agent + analytics), `KnowledgeBase`, `Automation` (triggers + conditions + actions), `Surveys`
- **Disposition:** `Typification` (first-class schema-driven cascading / conditional / AI disposition module; clean-break of the flat `Disposition`)
